    ...
    <a-skeleton :paragraph="{rows:12}" :loading="loading" active>
      <div v-if="comment_list.length !== 0" class="comment-list">
        <div class="inner-container">
          <div v-for="item of current_list" v-bind:key="item.comment.id" class="container-card">
            <a-divider/>
            <comment :comment_id="item.comment.id" :figure_id="item.comment.figure_id" :comment_time="item.comment.created_at"
                     :comment_video="object_info.obj_id" :commenter="item.user_name" :content="item.comment.content"
  ...
  methods: {
    ...
    update_comments() {
      ...
      if (this.object_info.obj_key === 'figureId') {
        reURL = `figures/${this.object_info.obj_id}/comments`;
      }else if(this.object_info.obj_key === 'username') {
        reURL = `select/figure_comments?user_name=${this.object_info.obj_id}`;
      }
      api_service.get(reURL)
          .then((res) => {
            console.log(res);
    ...
    onDelComment(comment_id) {
      this.comment_list = this.comment_list.filter(item => (item.comment.id !== comment_id));
    ...
    handleSubmit() {
      ...
      let _this = this;
      let reqURL = `figures/${this.object_info.obj_id}/comments`;
