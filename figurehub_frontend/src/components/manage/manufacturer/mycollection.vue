...
export default {
  ...
  methods: {
    ...
    onDelete(key) {
      console.log(key);
      axios_service.delete_with_params(this.$store, "/companies/" + key, null).then((res) => {
        console.log(res);
        let _this = this;
        _this.loading = true;
        _this.queryTable();
    ...
    searchModal(value) {
      if (!value || value == "") {
        ...
        let _this = this;
        _this.loading = true;
        axios_service.get_with_params(this.$store, "/search/companies", params).then((res) => {
              //将返回的数据存入页面中声明的data中
              _this.data = res;
              _this.loading = false;
            })
    ...
    updateCreate() {
      ...
      form.validateFields((err, values) => {
        ...
        axios_service.put_with_params(this.$store, "/companies/" + values.id, params).then((res) => {
              console.log(res);
              let _this = this;
              _this.loading = true;
              _this.queryTable();
    ...
    handleCreate() {
      ...
      form.validateFields((err, values) => {
        ...
        axios_service.post_with_params(this.$store, "/companies", params).then((res) => {
              let _this = this;
              _this.loading = true;
              _this.queryTable();
            });
