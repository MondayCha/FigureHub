      ...
      <a-form-model :model="form" :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-form-model-item label="手办名称">
          <a-select @change="selectChange" placeholder="please select your zone">
            <a-select-option v-for="figure in figureList" :value="figure.id" :key="figure.id">
              {{ figure.name }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
        <a-form-model-item has-feedback label="价格" prop="number">
          <a-input prefix="￥" v-model="form.price" type="number" autocomplete="off"/>
        </a-form-model-item>
        <a-form-model-item has-feedback label="数量" prop="number">
          <a-input v-model="form.status" type="number" autocomplete="off"/>
        </a-form-model-item>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      axios_service.post_with_params(this.$store, "merchandises", params).then((res) => {
        let mid = res.id.toString();
        let params2 = {
          merchandise_id: mid,
          figure_id: _this.form.selectFigure,
        };
        axios_service.post_with_params(this.$store, "sales", params2).then(() =>{
    ...
    axios_upload(index, uid, formList) {
      ...
      let _this = this;
      return axios_service.post_with_image(this.$store, `merchandises/${uid}/images`, null, formList[index].originFileObj).then((res) => {
